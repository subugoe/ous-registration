...
if ($_SESSION["email"] != "")
{
  ...
  // usertype_id um 20 für die e-mail-kennzeichnung  erhöhen 
 ...
 $q = "UPDATE persons set last_name='@last_name@', " . 
   ...
   "student_id='@student_id@' where id=@id@";//CE ERG
}
else
{
 $kw[ '@' . email_checkbox . '@'] = 'f';
...
} else if (($_SESSION["action"] == "delete") and 	
...
else if ($_SESSION["button"] == $buttons['goto_index'] ) {
...
else if (($_SESSION["button"] == $buttons['submit']) or 
	( $_SESSION['button'] == "" and $_SESSION['barcode'] != "" ) ) {

	$_SESSION["action"] = "submit"; 	// switch to "submit" mode
}
else if ($_SESSION["button"] == $buttons['delete'])	
	$_SESSION["action"] = "delete";		// switch to "delete" mode
	
 else if ($_SESSION["button"] == $buttons['print'])	
 
 
 $_SESSION["action"] = "print";		// switch to "print" mode
...
} else if (isset($_SESSION["button"]))
	$_SESSION["action"] = "show";		// switch to "show" mode 
